...
static unsigned char ipv4_mapped_ipv6_address_prefix[] =
...
datagram_spec *get_ds(datagram_spec * ds, socket_msg * sm)
{
	...
	if (getsockopt
	    (sm->sockfd, SOL_SOCKET, SO_TYPE, &(ds->so_type), &length)) {
		perror("socket_type");
	}
	switch (ds->so_type) {
	case SOCK_STREAM:
	...
	for (cmptr = CMSG_FIRSTHDR(&sm->msg); cmptr != NULL;
		...
		if (cmptr->cmsg_level == IPPROTO_IPV6 &&
			...
			struct in6_pktinfo *pkt =
			    (struct in6_pktinfo *)CMSG_DATA(cmptr);
			struct in6_addr *dap = &(pkt->ipi6_addr);
			inet_ntop(AF_INET6,
				  (void *)dap, ds->pladdr, sizeof(ds->pladdr));
	...
	inet_ntop(AF_INET6,
		  (void *)&(raddr->sin6_addr), ds->praddr, sizeof(ds->praddr));
...
ssize_t socket_recvmsg(socket_msg * smp)
{
	...
	result =
	    setsockopt(smp->sockfd, IPPROTO_IPV6, IPV6_RECVPKTINFO, &opt,
		       sizeof(opt));
	if (result) {
		perror("setsockopt");
	}
	if (0 > (result = recvmsg(smp->sockfd, &(smp->msg), 0))) {
		...
		zlog_info(zc, "rcvd %d bytes %s [%s]:%hu to [%s]:%hu",
			  result,
			  ds.sock_type,
			  ds.praddr, ds.rport, ds.pladdr, ds.lport);
	}
	return result;
}				// socket_recvmsg()
...
ssize_t socket_sendmsg(socket_msg * sm)
{
	...
	zlog_info(zc, "sent %zu bytes %s [%s]:%hu to [%s]:%hu",
		  result,
		  ds.sock_type, ds.pladdr, ds.lport, ds.praddr, ds.rport);
	return result;
}				// socket_sendmsg()
